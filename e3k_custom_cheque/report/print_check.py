...
class report_print_check(models.Model):
  ...
  def _compute_is_e3k_partial_payment_installed(self):
    for payment in self:
      is_installed = False
      module_installed = self.env['ir.module.module'].sudo().search([('name', '=', 'e3k_partial_payment'), ('state', '=', 'installed')])
      if module_installed:
        is_installed = True
  ...
  def _check_build_page_info(self, i, p):
    ...
    payment_date_1 = fields.Date.from_string(self.payment_date)
    ...
    if not self.partner_id.lang:
      raise ValidationError(_("S'il vous plaît, veuillez renseigner la langue du partenaire dans sa fiche de contact avant d'imprimer le chèque"))
    ...
    page.update({
      ...
      'payment_date_canada': format_date(self.env, self.payment_date, str(self.company_id.date_format)),
  ...
  def _check_make_stub_line_payment(self, payment):
    ...
    return {
      ...
      'amount_total': formatLang(self.env, payment.amount_total_signed, currency_obj=payment.currency_id),
      'amount_residual': formatLang(self.env, payment.amount_residual_signed, currency_obj=payment.currency_id) if payment.amount_residual_signed * 10 ** 4 != 0 else '-',
      'amount_paid': formatLang(self.env, payment.partial_payment, currency_obj=payment.currency_id),
  ...
  def _check_make_stub_pages(self):
    ...
    if (self.is_e3k_partial_payment_installed):
      stub_lines = [self._check_make_stub_line_payment(pay) for pay in
                    self.payment_partial_ids.sorted(key=lambda r: (r.invoice_date_due, r.id))]
    else:
